...
**The dominant register should usually be the most-frequent superior
emotion across your beat tags.** Compute this from the beats you have
written: count how often each superior emotion (`wonder`, `dread`,
`grief`, `triumph`, `warmth`, `moral_weight`, `tension`, `joy`) appears
across all of your beat tags. Default the dominant register to the most
frequent superior. Do not pick a register based on atmospheric vibe when
your own tagging supports a different answer.

If you choose a dominant register that is NOT the most-frequent superior
in your beats, you must justify it in the `justification` field of your
final (endpoint) beat. Note specifically why beat-tag frequency does not
reflect the film's actual background register. Valid reasons include:

- A film whose background register is genuinely atmospheric rather than
  structural. Example: Bajrangi Bhaijaan's beats are heavily tagged with
  `tension.*` (border crossings, jail) but the background register is
  `warmth` — the film's emotional gravity is tenderness, even when the
  scenes are tense.
- A film whose register is carried by music, color, or pacing rather
  than discrete tagged moments.

If you cannot articulate a specific reason, default to beat-tag
frequency. Atmospheric reads are the exception, not the default.

**Concrete example:** The Dark Knight contains many `dread.*` tags
because individual scenes feel oppressive. But across the full beat set,
`moral_weight.*` and `tension.*` likely appear more frequently because
the film's structural spine is ethical pressure and suspended outcomes.
The dominant register for Dark Knight should be `moral_weight` or
`tension` (whichever you tagged most), not `dread`, unless you can
articulate why dread is structurally — not atmospherically — dominant.
